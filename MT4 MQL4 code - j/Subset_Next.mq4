...
//goto next currency pair in array
...
{  
   ...
   int sI = GlobalVariableGet("Subset");
   sI++; 
   if (sI > 19) {sI = 0;}    //resets for scroll effect
   ...
   //Alert(sI," ",SymbolArray[sI]); Sleep(5000);
   ...
   if (sI == 0) {PlaySound("bell.wav");}        //signals when last currency pair in subset is reached
   
   GlobalVariableSet("Subset",sI);
   return;
}
